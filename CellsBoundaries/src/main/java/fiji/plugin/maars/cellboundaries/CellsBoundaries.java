...
public class CellsBoundaries implements PlugIn{
	...
	private JComboBox maxParticleSizeComboUnit;
		...
		// to notify if the cells boundaries are white then black or black then white
	...
	public void setMainWindow() {
		...
		Panel prefPanel = new Panel();
			...
			prefPanel.setLayout(prefLayout);
				...
				BoxLayout changeScaleBoxLayout = new BoxLayout(changeScalePanel, BoxLayout.X_AXIS);
				...
				BoxLayout filterUnusualLayout = new BoxLayout(filterUnusualPanel, BoxLayout.X_AXIS);
				filterUnusualPanel.setLayout(filterUnusualLayout);
					filterUnususalCkb = new Checkbox("Filter unusual shape using solidity", filter);
				...
				BoxLayout filterWithMinGrayValueLayout = new BoxLayout(filterWithMinGreyValuePanel, BoxLayout.X_AXIS);
				filterWithMinGreyValuePanel.setLayout(filterWithMinGrayValueLayout);
					filterWithMeanGreyValueCkb = new Checkbox("Filter background using mean grey value on correlation image", filter);
				...
				// Determine minimum cell area and maximum cell area allowed when analysing particles
				Panel setMinMaxPaticleSizePanel = new Panel();
				BoxLayout setMinMaxPaticleSizeLayout =  new BoxLayout(setMinMaxPaticleSizePanel, BoxLayout.X_AXIS);
				setMinMaxPaticleSizePanel.setLayout(setMinMaxPaticleSizeLayout);
					...
					BoxLayout minMaxPartSizeLayout = new BoxLayout(minMaxPartSizePanel, BoxLayout.Y_AXIS);
				...
				// notify if the cells boundaries are white then black or black then white
				Panel blackOrWhitePanel = new Panel();
				BoxLayout blackOrWhiteLayout = new BoxLayout(blackOrWhitePanel, BoxLayout.Y_AXIS);
				blackOrWhitePanel.setLayout(blackOrWhiteLayout);

				Label blackOrWhiteLabel = new Label("Evolution of cells boundries on the movie :");

				String blackOrWhiteState[] = {"First slice black - Last slice white","First slice white - Last slice black"};
				...
				//BoxLayout preciseZFocusLayout = new BoxLayout(preciseZFocusPanel, BoxLayout.X_AXIS);

				preciseZFocusCheckbox = new Checkbox("Precise the slice corresponding to focus (default is the middle one)");
		...
		resultPanel.setBackground(Color.WHITE);
			...
			BoxLayout displayBoxLayout = new BoxLayout(displayPanel, BoxLayout.Y_AXIS);
			BoxLayout saveBoxLayout = new BoxLayout(savePanel, BoxLayout.Y_AXIS);

			BoxLayout labDispSaveLayout= new BoxLayout(labDispSavePanel, BoxLayout.X_AXIS);
			BoxLayout folderPathLayout = new BoxLayout(folderPathPanel, BoxLayout.Y_AXIS);
		...
		// Panel where is shown name of the file which is going to be used for algorithm
		...
		runPanel.setLayout(runLayout);
			...
			BoxLayout getPicLayout = new BoxLayout(getPicturePanel, BoxLayout.Y_AXIS);
	...
	public void run(String arg) {
		...
		/** IN CASE YOU JUST WANT TO RUN WITH A MACRO (you open the image then run this plugin)
		 * UN-COMMENT THIS and COMMENT mainWindow.showDialog();
		 *              |
		 *              V
		 */
...
//		CellsBoundariesIdentification cBI = new CellsBoundariesIdentification(this, 13, 200, 900000, -1, (int) Math.round(imageToAnalyze.getSlice()/2),0.84 ,-177660);
