...
public class RunAction implements ActionListener {
	...
	public boolean checkResultOptions() {
		...
		return (cB.getDisplayCorrelationImg().getState()
				...
				|| cB.getSaveFocusImage().getState() || cB.getSaveRoi()
				.getState());
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (fileNameField.getText().isEmpty()) {
			...
			System.out
					.println("Check if none of the result ckeckBox is selected");
			...
			if (!thereIsAResult) {
				...
				System.out
						.println("check if typical cell Z size field has been filled");
				if (cB.getSizeField().getText().isEmpty()) {
					IJ.error("Missing parameter",
							"You need to fill the typical cell Z size field");
				} else {

					// if the unit chosen is a micron it must be converted
					System.out
							.println("Check if one of the unite used is micron");
					if (CellsBoundaries.MICRONS == cB.getSizeComUnit()
							.getSelectedIndex()
							|| CellsBoundaries.MICRONS == cB.getMaxWComboUnit()
									.getSelectedIndex()
							|| CellsBoundaries.MICRONS == cB.getMaxHComboUnit()
									.getSelectedIndex()
							|| CellsBoundaries.MICRONS == cB
									.getMinParticleSizeComboUnit()
									.getSelectedIndex()
							|| CellsBoundaries.MICRONS == cB
									.getMaxParticleSizeComboUnit()
									.getSelectedIndex()) {
						checkUnitsAndScale();
					}
					...
					try {
						typicalCellSize = Double.parseDouble(cB.getSizeField()
								.getText());
					} catch (NumberFormatException nfe) {
						IJ.error("Wrong parameter",
								"The typical cell size is supposed to be a number");
					}
					// Check if the cell size is a size
					if (typicalCellSize <= 0) {
						IJ.error("Wrong parameter",
								"The typical cell size must be a positive, not null value");
					} else {
						...
						if (CellsBoundaries.MICRONS == cB.getSizeComUnit()
							...
							if (CellsBoundaries.PIXELS == cB.getSizeComUnit()
									.getSelectedIndex()) {
						...
						try {
							minParticleSizeTemp = Double.parseDouble(cB
									.getMinParticleSizeField().getText());
						} catch (NumberFormatException nfe) {
							IJ.error("Wrong parameter",
									"The minimum area is supposed to be a number");
						}
						...
						if (minParticleSizeTemp <= 0) {
							...
							System.out
									.println("Try to read maximum particle size");
							double maxParticleSizeTemp = 0;

							try {
								maxParticleSizeTemp = Double.parseDouble(cB
										.getMaxParticleSizeField().getText());
							} catch (NumberFormatException nfe) {
								IJ.error("Wrong parameter",
										"The maximum area is supposed to be a number");
							}
							...
							if (maxParticleSizeTemp <= 0) {
								...
								System.out
										.println("Check if user wants to change scale");
								if (cB.getScaleCkb().getState()) {
									...
									System.out
											.println("Try to read width value");
									try {
										newMaxWidth = Double.parseDouble(cB
												.getMaxWTextField().getText());
									} catch (NumberFormatException nfe) {
										IJ.error("Wrong parameter",
												"The maximum width is supposed to be a number");
									}
									...
									if (newMaxWidth <= 0) {
										...
										System.out
												.println("Try to read height value");
										try {
											newMaxHeight = Double
													.parseDouble(cB
															.getMaxHTextField()
								...
								if (cB.getPreciseZFocusCheckbox().getState()) {
									...
									try {
										zf = Float.parseFloat(cB
												.getPreciseZFocusTextField()
												.getText());
									} catch (NumberFormatException nfe) {
										IJ.error(
												"Wrong parameter",
								...
								System.out
										.println("Check if user want to filter shape using solidity");
								if (cB.getFilterUnususalCkb().getState()) {
									double sldtThrshld = 0;

									System.out
											.println("Try to read solidity threshold value");
									try {
										sldtThrshld = Double.parseDouble(cB
												.getSolidityField().getText());
									} catch (NumberFormatException nfe) {
										IJ.error(
												"Wrong parameter",
								...
								if (cB.getFilterWithMeanGreyValueCkb()
										.getState()) {
									...
									try {
										mnGryVlThrshld = Double
												.parseDouble(cB
														.getMeanGreyValueThresholdField()
														.getText());
									} catch (NumberFormatException nfe) {
										IJ.error(
												"Wrong parameter",
