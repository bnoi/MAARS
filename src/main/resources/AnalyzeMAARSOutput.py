...
def getMitoticElongation(cellRois, calibration, acq_interval, features_dir, cellNb, p, minSegLen, channel,
                         mitosisFigDir, save_plot=True):
                ...
                if np.isnan(spAng2MajVal):
                    if last_spot_0_x_y == None:
                        ...
                        if last_spot_1_x_y == None:
                            ...
                            dis00 = distance(last_spot_0_x_y[0], last_spot_0_x_y[1], 0, current_spot_0_x,
                                             current_spot_0_y, 0)
                            dis10 = distance(last_spot_1_x_y[0], last_spot_1_x_y[1], 0, current_spot_0_x,
                                             current_spot_0_y, 0)
                            if dis00 < dis10:
                                concat_data.append(np.array([i,
                                                             cell_major_length,
                    ...
                    if last_spot_0_x_y == None:
                        ...
                        dis00 = distance(last_spot_0_x_y[0], last_spot_0_x_y[1], 0, current_spot_0_x, current_spot_0_y,
                                         0)
                        dis01 = distance(last_spot_0_x_y[0], last_spot_0_x_y[1], 0, current_spot_1_x, current_spot_1_y,
                                         0)
                        if dis00 < dis01:
                            concat_data.append(np.array([i,
                                                         cell_major_length,
...
def analyse_each_cell(pool, save, anaphase_elongations, figureDir, cellRois, mitosisDirfeatures, fluo_suffix, features,
                      channel, spots):
...
if __name__ == '__main__':
    ...
    copy_mitosis_files(anaphase_elongations, ["CFP", "GFP", "TxRed", "DAPI"], fluoDir, mitosisDir, cropImgs, spots,
                       features)
    analyse_each_cell(pool, True, anaphase_elongations, mitosisFigDir, cellRois, mitosisDir, fluo_suffix, features,
                      channel, spots)
    print("Done")
